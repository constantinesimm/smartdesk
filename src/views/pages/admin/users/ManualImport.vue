<template>
  <d-container fluid class="admin-content-container px-4">

    <!-- Page Header -->
    <d-row no-gutters class="page-header py-3">

      <!-- Page Title -->
      <d-col col sm="4" class="text-center text-sm-left mb-4 mb-sm-0">
        <span class="text-uppercase page-subtitle">Users Manage</span>
        <h3 class="page-title">Manual import</h3>
      </d-col>
    ...
    <v-client-table :data="tableData" :columns="columns" :options="clientTableOptions" class="dataTables_wrapper file-manager file-manager-list">
      ...
      <template slot="beforeTable">
        <file-pond allow-multiple="true" max-files="3"
                   styleButtonRemoveItemPosition="top-left"
                   ref="pond" class-name="my-pond"
                   instantUpload="false"
                   label-idle="Drop files here or <span class='btn btn-sm btn-pill btn-outline-primary' style='padding: 0.15rem 0.75rem;'>Browse</span>"
                   v-bind:files="myFiles"
                   v-on:init="handleFilePondInit" v-on:addfile="handleAddFile"/>
      </template>
...
<script>
  import "../../../../assets/scss/vue-tables.css"
  ...
  export default {
    ...
    methods: {
      handleFilePondInit: function () {
        ...
        this.$refs.pond.fileType();
      },
      handleAddFile: function () {
        console.log('File added', this.$refs.pond.getFile(1).fileType());

        this.$refs.pond.getFile('12345')
      },
